...
class MVRakLibInterface extends RakLibInterface{
	...
	public function __construct(Server $server, ProtocolAcceptor $acceptor, string $ip, int $port, bool $ipV6){
		...
		ReflectionUtils::setProperty(RakLibInterface::class, $this, "broadcaster", new StandardPacketBroadcaster(
			$server
		));
	}
	...
	public function onClientConnect(int $sessionId, string $address, int $port, int $clientID) : void{
		$session = new MVNetworkSession(
			...
			ReflectionUtils::getProperty(RakLibInterface::class, $this, "broadcaster"),
			$address,
			$port
		);
